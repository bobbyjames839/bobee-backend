...
router.post('/', async (req, res) => {
    ...
    try {
        const { userId, transcript, prompt = '', aiResponse, timerSeconds, } = req.body;
        // — validate
        ...
        if (userSnap.exists) {
            ...
            const lastDate = fsDate && typeof fsDate.toDate === 'function'
                ? fsDate.toDate()
                : null;
            if (lastDate) {
        ...
        await userRef.set({
            ...
            lastJournalDate: firebase_admin_1.default.firestore.Timestamp.fromDate(new Date()),
        }, { merge: true });
